...
class medt_net(nn.Module):
    ...
    def __init__(self, block, block_2, layers, num_classes=2, zero_init_residual=True,
                 groups=8, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, s=0.125, img_size=128, imgchan=3, patch_num=4):
        ...
        self.patch_step = img_size // patch_num
        self.counter = 0
        ...
        self.conv1 = nn.Conv2d(imgchan, self.inplanes, kernel_size=7, stride=2, padding=3,
                               bias=False)
        self.conv2 = nn.Conv2d(self.inplanes, 32, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv3 = nn.Conv2d(32, self.inplanes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = norm_layer(self.inplanes)
        ...
        self.conv1_p = nn.Conv2d(imgchan, self.inplanes, kernel_size=7, stride=2, padding=3,
                                 bias=False)
        self.conv2_p = nn.Conv2d(self.inplanes, 32, kernel_size=3, stride=1, padding=1,
                                 bias=False)
        self.conv3_p = nn.Conv2d(32, self.inplanes, kernel_size=3, stride=1, padding=1,
                                 bias=False)

        self.bn1_p = norm_layer(self.inplanes)
        self.bn2_p = norm_layer(32)
        ...
        # Decoder
        self.decoder2_p = nn.Conv2d(int(512 *2*s), int(512*2*s), kernel_size=3, stride=2, padding=1)
        self.decoder3_p = nn.Conv2d(int(512*2*s), int(512*s), kernel_size=3, stride=1, padding=1)
    ...
    def _make_layer(self, block, planes, blocks, kernel_size=56, stride=1, dilate=False, multiplicator=True):
        norm_layer = self._norm_layer
        downsample = None
        previous_dilation = self.dilation
        if dilate:
    ...
    def _forward_impl(self, x):
        ...
        for i in range(self.patch_num):
            for j in range(self.patch_num):
                ...
                w = self.patch_step * i
                h = self.patch_step * j
                x_p = xin[:, :, w:w+self.patch_step, h:h+self.patch_step]
                ...
                x_loc[:, :, w:w+self.patch_step, h:h+self.patch_step] = x_p
